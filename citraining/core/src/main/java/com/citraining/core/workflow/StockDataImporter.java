...
public class StockDataImporter implements Importer {
	private final Logger LOGGER = LoggerFactory
			.getLogger(StockDataImporter.class);
	private final String SOURCE_URL = "http://finance.yahoo.com/d/quotes.csv?f=snd1lyr&s=";

	@Override
	public void importData(final String schema, final String dataSource,
		...
		try {
			...
			String lastTrade = Arrays.asList(
					Pattern.compile(",").split(readLine)).get(3);
			LOGGER.error("last trade for stock {} was {}", dataSource,
					lastTrade);
			in.close();
	...
	private void writeToRepository(String stockSymbol, String lastTrade,
		...
		PageManager pm = resource.getResourceResolver().adaptTo(
				PageManager.class);

		LOGGER.debug("parent path::::::::" + parent.getPath());
		LOGGER.debug("stockSymbol::::::::::::" + stockSymbol);
		try {
			Page stockdetails = pm.create(parent.getPath(), stockSymbol,
					"/apps/citraining/templates/page-content", "Stockpage");
			Node contentRes = stockdetails.getContentResource().adaptTo(
					Node.class);
			LOGGER.debug("Path of contentRes::::::::::::"
					+ contentRes.getPath());
			Node pageLastTradeNode = contentRes.addNode("par/lastTrade",
					"nt:unstructured");
			...
			pageLastTradeNode.setProperty("sling:resourceType",
					"citraining/components/content/text");
		...
		/*
		 * Node
		 * stockPageNode=JcrUtil.createPath(parent.getPath()+"/"+stockSymbol,
		 * "cq:Page", parent.getSession()); Node
		 * lastTradeNode=JcrUtil.createPath
		 * (stockPageNode.getPath()+"/lastTrade", "nt:unstructured",
		 * parent.getSession()); lastTradeNode.setProperty("lastTrade",
		 * lastTrade); lastTradeNode.setProperty("lastUpdate",
		 * Calendar.getInstance()); parent.getSession().save();
		 */
	}
