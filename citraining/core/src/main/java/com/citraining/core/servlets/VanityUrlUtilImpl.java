...
public class VanityUrlUtilImpl extends SlingAllMethodsServlet {
	...
	private void getAllVanityPaths(SlingHttpServletRequest request,
		...
		try {
			...
			HashMap<String, String> duplicateVanityList = getDuplicateVanity(
					session, rr);

			TidyJSONWriter tidyJSONWriter = new TidyJSONWriter(
					response.getWriter());
			...
			for (int i = 0; i < nodes.getSize(); i++) {
				...
				if (!duplicateVanityList.isEmpty()) {
					log.info("####### duplicate Key ######## : "
							+ duplicateVanityList.get(site
									+ page.getVanityUrl()));
					if (duplicateVanityList.containsKey(site
							+ page.getVanityUrl())) {
				...
				if (node.hasProperty("sling:redirect")) {
					String redirect = node.getProperty("sling:redirect")
							.getString();
					log.info("####### redirect ######## : " + redirect);
					if (node.hasProperty("sling:redirectStatus")) {
						String redirectStatus = node.getProperty(
								"sling:redirectStatus").getString();
						log.info("####### redirectStatus ######## : "
								+ redirectStatus);
	...
	private void getVanityPath(SlingHttpServletRequest request,
			SlingHttpServletResponse response) {
		Session session = request.getResourceResolver().adaptTo(Session.class);
		try {
			...
			try {
				...
				NodeIterator nodes = getVanityPaths(session, rootPath,
						vanityPath);
	 ...
	 */
	private Page getPageObject(ResourceResolver rr, Node node)
			throws RepositoryException {
		log.info("##### NOde Path ####### : " + node.getPath() + " : "
				+ node.getPath().replace("/jcr:content", ""));
		Resource resource = rr.getResource(node.getPath().replace(
				"/jcr:content", ""));
		Page page = resource.adaptTo(Page.class);
		return page;
	}

	private HashMap<String, String> getDuplicateVanity(Session session,
			ResourceResolver rr) throws RepositoryException {
		...
		for (int i = 0; i < itr.getSize(); i++) {
			...
			if (map.containsKey(site + page.getVanityUrl())) {
				duplicateVanity.put(site + page.getVanityUrl(),
						page.getVanityUrl());
			} else {
	 ...
	 */
	private NodeIterator getVanityPaths(Session session, String path,
			String vanityPath) throws RepositoryException,
			PathNotFoundException, UnsupportedRepositoryOperationException,
			InvalidQueryException {
		log.info("####### session ######### : " + session.getUserID());
		Node root = session.getRootNode();
		Node currentNode = root.getNode("content");
		QueryObjectModelFactory qf = currentNode.getSession().getWorkspace()
				.getQueryManager().getQOMFactory();
		Selector selector = qf.selector("nt:base", "s");
		Constraint constriant = qf.descendantNode("s", path);
		constriant = qf.and(constriant,
				qf.propertyExistence("s", "sling:vanityPath"));
		if (vanityPath != null) {
			ValueFactory valueFactory = session.getValueFactory();
			String operator = QueryObjectModelConstants.JCR_OPERATOR_EQUAL_TO;
			DynamicOperand dynOperand = qf.propertyValue("s",
					"sling:vanityPath");
			StaticOperand statOperand = qf.literal(valueFactory
					.createValue(vanityPath));
			constriant = qf.and(constriant,
					qf.comparison(dynOperand, operator, statOperand));
		}
	...
	public void doPost(SlingHttpServletRequest request,
		...
		try {
			...
			if (operation.equals("update")) {
				...
				log.error("######## vanityurl ########" + vanityurl + " : "
						+ request.getParameter("vanityurl"));
				...
				for (int i = 0; i < vanitypaths.length; i++) {
					...
					NodeIterator nodes = getVanityPaths(session, site,
							values[i].getString());
					if (nodes.getSize() > 0) {
						while (nodes.hasNext()) {
							Node node = nodes.nextNode();
							...
							log.error("######## node path ######## : "
									+ node.getPath());
							if (!(node.getPath().replace("/jcr:content", "")
									.equals(path))
									&& (nodeSite.equals(pageSite))) {
								isDup = true;
								...
								tidyJSONWriter
										.key("Error")
										.value("This Vanity Path already exists in this Site");
							}
						}
					}
				}
				if (!isDup) {
					...
					tidyJSONWriter.key("Success").value(
	...
	private void updateVanityUrl(Session session, String path,
			Value[] vanityurl, String redirectType)
			throws PathNotFoundException, RepositoryException {
		log.info("####### Inside updateVanityUrl ########");

		Node currentNode = session.getNode(path);
		currentNode.getNode("jcr:content").setProperty("sling:vanityPath",
				vanityurl);
		if (!redirectType.equals("1")) {
			currentNode.getNode("jcr:content").setProperty("sling:redirect",
					true);
			currentNode.getNode("jcr:content").setProperty(
					"sling:redirectStatus", redirectType);
		}

	}

	private void deleteVanityUrl(Session session, String path)
			throws PathNotFoundException, RepositoryException {
		...
		if (currentNode.getNode("jcr:content").hasProperty(
				"sling:redirectStatus")) {
			currentNode.getNode("jcr:content").setProperty(
					"sling:redirectStatus", v);
		}
