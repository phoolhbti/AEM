...
public class SearchUtils {
	...
	public static String getLastMoidifedPage(final Session session,
			String path, String template) {
		String sqlStatement = "SELECT parent.* FROM [cq:Page] AS parent INNER JOIN [nt:base] AS child ON ISCHILDNODE(child,parent) WHERE ISDESCENDANTNODE(parent, '"
				+ path
				+ "')"
				+ " AND child.[cq:template] = '"
				+ template
				+ "' " + "ORDER BY child.[cq:lastModified] desc";
		QueryManager qm = getQueryManger(session);
		try {
			javax.jcr.query.Query query = qm.createQuery(sqlStatement,
					"JCR-SQL2");
