...
public @interface SolrConfiguration {
	...
	@AttributeDefinition(name = "Solr Core Name",defaultValue="citraining", description = "Core name in solr server")
	String getSolrCoreName();
	
	@AttributeDefinition(name = "Content page path",defaultValue="/content/citraining", description = "Content page path from where solr has to index the pages e.g /content/geometrixx")
